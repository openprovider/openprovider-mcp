...
export const GetDomainArgs = z.object({ id: z.coerce.number().int().positive() });
...
export const GetContactArgs = z.object({ id: z.coerce.number().int().positive() });
...
export const CreateContactArgs = z
  .object({
    ...
    // Openprovider's contact create endpoint requires username + email
    // (see PostMan/OP Rest API collection; create_customer requires them too).
    username: z.string().min(1),
    email: z.string().email(),
    // Openprovider REQUIRES a role on create — omitting it is rejected upstream
    // as `{"code":170,"desc":"Empty type field!"}` (the server's internal name
    // for this field is `type`). Only `tech` and `billing` are valid for the
    // /contacts endpoint; `owner`/`admin` are rejected as "Invalid customer
    // type". Verified live against the Openprovider API 2026-06-23.
    role: z.enum(['tech', 'billing']),
...
export const DomainIdArg = z.object({ id: z.coerce.number().int().positive() });
...
const DnsRecord = z.object({
  ...
  prio: z.coerce.number().int().nonnegative().optional(),
});
...
const NsGroupMember = z.object({
  ...
  ip: z.string().min(1),
  ip6: z.string().optional(),
  seq_nr: z.coerce.number().int().nonnegative(),
});
...
export const TemplateIdArg = z.object({ id: z.coerce.number().int().positive() });
...
// Path-id args
export const SslOrderIdArg = z.object({ id: z.coerce.number().int().positive() });
export type SslOrderIdArg = z.infer<typeof SslOrderIdArg>;

export const SslProductIdArg = z.object({ id: z.coerce.number().int().positive() });
...
/**
 * Repair a PEM blob (CSR/cert/key) whose newlines were escaped or stripped in
 * transit. This happens routinely when a PEM produced by one tool (e.g.
 * `create_csr`, which returns it inside a JSON string with `\n` escapes) is
 * round-tripped through an LLM/client into another tool: the newlines arrive
 * as literal `\n`, as spaces, or dropped entirely. Openprovider's CSR
 * endpoints reject such input as `code 1030 "Invalid CSR"` (verified live
 * 2026-06-24). We unescape literal `\n`/`\r\n`, then re-wrap the base64 body of
 * the first PEM block into canonical 64-char lines. Non-PEM input is returned
 * unchanged (after unescaping) so the upstream still produces its own error.
 */
export function normalizePemCsr(input: string): string {
  const unescaped = input.replace(/\\r\\n|\\n/g, '\n').trim();
  const m = unescaped.match(/-----BEGIN ([A-Z0-9 ]+)-----([\s\S]*?)-----END \1-----/);
...
/** A CSR/PEM string, repaired for mangled newlines before it reaches Openprovider. */
const PemCsrString = z.string().min(1).transform(normalizePemCsr);

const SslOrderBody = z.object({
  approver_email: z.string().min(1),
  autorenew: z.enum(['on', 'off']),
  csr: PemCsrString,
  domain_amount: z.coerce.number().int().nonnegative(),
  ...
  wildcard_domain_amount: z.coerce.number().int().nonnegative(),
});

export const CreateSslOrderArgs = SslOrderBody;
export type CreateSslOrderArgs = z.infer<typeof CreateSslOrderArgs>;

// An update only changes the supplied fields, so the create body is made
// optional here — requiring the full SslOrderBody (csr, approver_email, …) on
// every update was a bug (TestRail T21986). `id` stays required.
export const UpdateSslOrderArgs = SslOrderBody.partial().extend({
  id: z.coerce.number().int().positive(),
});
export type UpdateSslOrderArgs = z.infer<typeof UpdateSslOrderArgs>;

export const ReissueSslOrderArgs = SslOrderBody.extend({ id: z.coerce.number().int().positive() });
...
export const CancelSslOrderArgs = z.object({ id: z.coerce.number().int().positive() });
...
export const DecodeCsrArgs = z.object({ csr: PemCsrString });
export type DecodeCsrArgs = z.infer<typeof DecodeCsrArgs>;

// OTP token
export const CreateSslOtpTokenArgs = z.object({ id: z.coerce.number().int().positive() });
...
// path-arg schemas
export const EmailTemplateIdArg = z.object({ id: z.coerce.number().int().positive() });
export type EmailTemplateIdArg = z.infer<typeof EmailTemplateIdArg>;

export const EasyDmarcIdArg = z.object({ id: z.coerce.number().int().positive() });
...
export const RetryDmarcArgs = z.object({ id: z.coerce.number().int().positive() });
export type RetryDmarcArgs = z.infer<typeof RetryDmarcArgs>;

export const DmarcSsoLoginArgs = z.object({ id: z.coerce.number().int().positive() });
...
export const PleskKeyIdArg = z.object({ key_id: z.coerce.number().int().positive() });
...
const PleskLicenseBody = z.object({
  ...
  parent_key_id: z.coerce.number().int().nonnegative().optional(),
...
// Update only changes the supplied fields — requiring the full PleskLicenseBody
// (items, period, title, …) on every update was a bug (TestRail T21997).
// `key_id` stays required.
export const UpdatePleskLicenseArgs = PleskLicenseBody.partial().extend({
